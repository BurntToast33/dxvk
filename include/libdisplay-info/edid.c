...
static void
parse_version_revision(const uint8_t data[EDID_BLOCK_SIZE],
...
static size_t
parse_ext_count(const uint8_t data[EDID_BLOCK_SIZE])
{
	return data[0x7E];
}

static bool
validate_block_checksum(const uint8_t data[EDID_BLOCK_SIZE])
{
	uint8_t sum = 0;
	size_t i;
...
parse_vendor_product(struct di_edid *edid,
		     const uint8_t data[EDID_BLOCK_SIZE])
{
...
parse_standard_timing(struct di_edid *edid,
		      const uint8_t data[EDID_STANDARD_TIMING_SIZE],
...
_di_edid_parse_detailed_timing_def(const uint8_t data[EDID_BYTE_DESCRIPTOR_SIZE])
{
...
parse_display_range_limits(struct di_edid *edid,
			   const uint8_t data[EDID_BYTE_DESCRIPTOR_SIZE],
...
static bool
parse_ext(struct di_edid *edid, const uint8_t data[EDID_BLOCK_SIZE])
{
